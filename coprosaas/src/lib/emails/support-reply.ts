...
export function buildSupportReplyEmailSubject(subject: string): string {
  return `Réponse à votre demande — ${subject} — Mon Syndic Bénévole`;
}
...
export function buildSupportReplyEmail(params: SupportReplyEmailParams): string {
  ...
  const content = `
<h1 style="margin:0 0 6px;font-size:20px;font-weight:700;color:${COLOR.text}">Réponse à votre demande</h1>
...
<p style="margin:0 0 20px;font-size:14px;color:${COLOR.text};line-height:1.6">
  Notre équipe a répondu à votre demande. Voici son message&nbsp;:
</p>
...
${ctaButton('Consulter ma demande →', ticketUrl, COLOR.blue)}
  ...
  <tr>
    <td style="padding-top:16px">
      <p style="margin:0;font-size:13px;color:${COLOR.muted};line-height:1.6">
        Vous pouvez répondre directement depuis votre espace d'aide en cliquant sur le bouton ci-dessus.
      </p>
...
`;
  ...
  return wrapEmail(content, COLOR.blue, 'Notre équipe a répondu à votre demande');
}
