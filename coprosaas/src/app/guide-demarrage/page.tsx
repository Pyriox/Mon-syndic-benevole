...
const guideSteps: GuideStep[] = [
  {
    ...
    duration: '2 à 3 min',
    objective: 'Comprendre l’état initial réel de la plateforme : au départ, le compte est vide et il faut simplement créer la première copropriété.',
    actions: [
      'Après confirmation de votre adresse e-mail, vous arrivez sur un tableau de bord encore vide : c’est normal.',
      'Repérez le message de bienvenue et le bouton « Créer ma première copropriété ».',
      'Ne cherchez pas encore de chiffres, d’impayés ou de dépenses : ils n’apparaîtront qu’après la création de la copropriété et la saisie des premières données.',
    ],
    result: 'Vous comprenez tout de suite que rien n’est prérempli au départ et vous savez quelle première action lancer.',
    tips: [
      'Un compte neuf n’affiche aucune donnée financière tant que vous n’avez pas créé la copropriété et ajouté vos premiers éléments.',
      'Si vous gérez plusieurs immeubles plus tard, le sélecteur de copropriété en haut vous permettra ensuite de basculer rapidement de l’un à l’autre.',
    ],
    previewTitle: 'Tableau de bord — compte neuf',
    previewItems: ['Bienvenue', 'Aucune copropriété', 'Créer ma première copropriété', 'État initial réel'],
  },
  {
    id: 'copropriete',
    step: 'Étape 2',
...
function GuidePreview({ title, variant }: { title: string; variant: string }) {
  return (
    <div className="overflow-hidden rounded-2xl border border-slate-200 bg-white shadow-sm">
      <div className="flex items-center gap-2 border-b border-slate-100 bg-slate-50 px-4 py-3">
        ...
        <p className="ml-2 text-xs font-medium text-slate-500">Capture réelle de l’écran</p>
      </div>
      <div className="bg-slate-50 p-1.5 sm:p-2">
        <a href={`/guide-demarrage/captures/${variant}.png`} target="_blank" rel="noreferrer" className="block">
          <Image
            src={`/guide-demarrage/captures/${variant}.png`}
            alt={title}
            width={1280}
            height={900}
      ...
      <div className="border-t border-slate-100 px-4 py-2 text-[11px] text-slate-500">
        Cliquez sur la capture pour l’ouvrir en grand.
      </div>
    </div>
  );
}
            ...
            <div className="max-w-6xl mx-auto px-4 sm:px-6">
              ...
              <div className="mt-6 flex flex-wrap gap-3 text-sm">
                {[
                  ...
                  'Compte neuf et écran vide au départ',
                ].map((item) => (
              ...
              <div className="mt-5 max-w-3xl rounded-2xl border border-white/15 bg-white/5 px-4 py-3 text-sm text-blue-50/90">
                Les aperçus ci-dessous montrent <strong className="font-semibold text-white">l’état réel d’un compte neuf</strong> :
                au départ, il n’y a <strong className="font-semibold text-white">aucune donnée préremplie</strong> tant que la copropriété,
                les lots, les copropriétaires et les premières opérations n’ont pas été créés.
              </div>
              ...
              return (
                <section key={step.id} id={step.id} className="scroll-mt-24 rounded-3xl border border-slate-200 bg-white p-5 sm:p-6 shadow-sm">
                  <div className="grid gap-6">
                    ...
                    <GuidePreview title={step.previewTitle} variant={step.id} />
                  </div>
                </section>
              );
            })}
