        ...
        <div className="flex-1 min-w-0 flex flex-col bg-white rounded-2xl border border-gray-100 shadow-sm overflow-hidden" style={{ minHeight: '70vh' }}>
          ...
          {!selectedTicket ? (
            ...
            <>
              ...
              {/* Messages */}
              <div className="flex-1 overflow-y-auto overflow-x-hidden px-5 py-4 flex flex-col gap-3" style={{ minHeight: 0 }}>
                {loadingMsgs ? (
  ...
  return (
    <div className={`flex gap-2.5 ${isAdmin ? 'flex-row-reverse' : 'flex-row'}`}>
      ...
      <div className={`max-w-[80%] min-w-0 ${isAdmin ? 'items-end' : 'items-start'} flex flex-col gap-1`}>
        <div className={`px-3.5 py-2.5 rounded-2xl text-sm leading-relaxed whitespace-pre-wrap break-all ${
          isAdmin
