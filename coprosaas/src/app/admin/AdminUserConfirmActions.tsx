...
import { Loader2, MailCheck, MailOpen, ShieldCheck } from 'lucide-react';
  ...
  const handleResendWelcome = async () => {
    ...
    try {
      const res = await fetch('/api/admin/users', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ action: 'resend_welcome', email: userEmail, userId }),
      });
      if (!res.ok) {
        const data = await res.json().catch(() => ({})) as { error?: string };
        setWelcomeError(data.error ?? 'Erreur inconnue');
      } else {
        setWelcomeDone(true);
      }
    } finally {
  ...
  return (
    <>
      ...
      <button
        ...
        disabled={welcomeLoading || welcomeDone}
        className="inline-flex items-center gap-1.5 rounded-lg border border-blue-200 bg-white px-2.5 py-1.5 text-xs font-medium text-blue-700 hover:border-blue-300 disabled:opacity-60 transition-all"
        title="Réinitialise l'état d'idempotence et renvoie l'e-mail de bienvenue"
      >
        {welcomeLoading ? <Loader2 size={12} className="animate-spin" /> : <MailOpen size={12} />}
        {welcomeDone ? 'Bienvenue envoyé' : 'Renvoyer bienvenue'}
      </button>
      {welcomeError && <span className="text-xs text-red-600">{welcomeError}</span>}
